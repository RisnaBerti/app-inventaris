<div class="row mb-2">
            ...
            <select class="form-select @error('user_id') is-invalid @enderror" name="user_id" id="user-id"
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->id }}"
                        {{ isset($pegawai) && $pegawai->user_id == $user->id ? 'selected' : (old('user_id') == $user->id ? 'selected' : '') }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <input type="text" name="jabatan" id="jabatan"
                ...
                value="{{ isset($pegawai) ? $pegawai->jabatan : old('jabatan') }}" placeholder="{{ __('Jabatan') }}"
                required />
            @error('jabatan')
                <span class="text-danger">
                    {{ $message }}
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <input type="text" name="no_tlpn" id="no-tlpn"
                class="form-control @error('no_tlpn') is-invalid @enderror"
                value="{{ (isset($pegawai) ? $pegawai->no_tlpn : old('no_tlpn')) ? old('no_tlpn') : '-' }}"
                placeholder="{{ __('No Tlpn') }}" />
            @error('no_tlpn')
                <span class="text-danger">
                    {{ $message }}
                </span>
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <input type="text" name="alamat" id="alamat"
                ...
                value="{{ (isset($pegawai) ? $pegawai->alamat : old('alamat')) ? old('alamat') : '-' }}"
